...
def _emb():
    global _E
    try: _E
    except NameError: _E = SentenceTransformer(EMB_NAME)
    return _E
...
def _df_to_records(sym: str, df: pd.DataFrame, source: str) -> List[Tuple[str,str,str,float,str,int]]:
    if df is None or df.empty: return []
    out = []
    for row_name, row in df.iterrows():
        metric = str(row_name).strip().lower().replace(" ", "_")
    ...
    with _conn() as c:
        for sym in symbols:
            try:
                ...
                recs = []
                recs += _df_to_records(sym, fins, "yfinance:financials")
                recs += _df_to_records(sym, bs, "yfinance:balance_sheet")
                recs += _df_to_records(sym, cf, "yfinance:cashflow")
                if recs:
                    c.executemany("""
...
def numeric_retrieve(query: str, symbols: List[str], topk: int = 6) -> List[Dict[str, Any]]:
    ...
    for canon, _, r in scored:
        ...
        seen.add(key)
        out.append({
            "symbol": r["symbol"], "metric": canon, "period": r["period"], "value": r["value"],
            "source": r["source"], "url": "", "title": f"{canon} ({r['period']})"
        })
