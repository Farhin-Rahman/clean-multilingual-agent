...
.user-message, .assistant-message {
  padding: 1rem 1.5rem; border-radius: 1rem; margin-bottom: 1rem; line-height: 1.6;
  font-size: .95rem; box-shadow: 0 2px 8px rgba(0,0,0,.05); word-break: break-word;
}
...
.stTextArea textarea:focus {
  outline: none !important; border-color: #6366F1 !important; box-shadow: 0 0 0 2px rgba(99,102,241,.4) !important;
}
.stButton > button {
...
.demo-active {
  background-color: #d1fae5; color: #065f46; padding: .75rem 1rem; border-radius: .5rem; border: 1px solid #a7f3d0;
  font-size: .875rem; margin: 1rem 0;
}
.footer {
  background-color: #f7f7f8; border: 1px solid #e5e5e5; color: #8e8ea0; padding: .75rem 1rem; border-radius: .5rem;
...
with col2:
    if st.button("🚀 Send Message", use_container_width=True):
        if query.strip():
            ...
            chat_history = [
                {"user": st.session_state.messages[i]["content"], "agent": st.session_state.messages[i+1]["content"]}
                for i in range(0, len(st.session_state.messages) - 2, 2)
            ]

            # Construct user profile and requirements from the UI
            user_profile = {}
            user_requirements = {}
            # Check if financial mode is enabled before sending financial-specific data
            if st.session_state.get("is_financial_mode") and is_financial_query(query):
                ...
                # Intelligent weight mapping based on risk tolerance
                risk = user_profile["risk_tolerance"]
                if risk == "Aggressive":
                    user_requirements["weights"] = {"valuation":0.15,"risk":0.20,"momentum":0.35,"size":0.15,"sentiment":0.15}
                elif risk == "Conservative":
                    user_requirements["weights"] = {"valuation":0.30,"risk":0.40,"momentum":0.10,"size":0.15,"sentiment":0.05}
                else: # Moderate
                    user_requirements["weights"] = {"valuation":0.25,"risk":0.30,"momentum":0.20,"size":0.15,"sentiment":0.10}

            with st.spinner("🤖 Analyzing with multi-RAG and live data…"):
                # This is the new, correct function call
                result = run_customer_support(
                    query=query + appended_text,
                    user_profile=user_profile,
                    user_requirements=user_requirements,
                    force_language=lang_map[display_language],
                    chat_history=chat_history,
                    file_path=file_path,
                    user_id=st.session_state.user_id
                )
            # --- End of the correct logic ---
